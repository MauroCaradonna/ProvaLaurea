...
public class Model {
	 ...
	 */
	private void getPastPeriodCostVarianceData(Map<Item, CostVarianceData> pastCVDByItemMap, Map<String, HashMap<Item, CostVarianceData>> pastCVDBySiteMap, Map<Buyer, HashMap<Item, CostVarianceData>> pastCVDByBuyerMap){
		...
		for(Order o : this.pastPeriodOrderMap.values()){
			...
			if(pastCVDByItemMap.containsKey(item) ){
				...
				pastCVDByItemMap.put(item, new CostVarianceData(item, invoice.getTotalPurchasePrice(), invoice.getQuantity(), invoice.getUnitPurchasePrice(), purchaseDate) );
			}
			...
			if(pastCVDBySiteMap.containsKey(siteId)) {
				...
				if(thisPastCVDByItemMap.containsKey(item) ){
					...
					thisPastCVDByItemMap.put(item, new CostVarianceData(item, invoice.getTotalPurchasePrice(), invoice.getQuantity(), invoice.getUnitPurchasePrice(), purchaseDate) );
				}
			} else {
				pastCVDBySiteMap.put(siteId, new HashMap<Item, CostVarianceData>());
				pastCVDBySiteMap.get(siteId).put(item, new CostVarianceData(item, invoice.getTotalPurchasePrice(), invoice.getQuantity(), invoice.getUnitPurchasePrice(), purchaseDate) );
			}
			...
			if(pastCVDByBuyerMap.containsKey(buyer)) {
				...
				pastCVDByBuyerMap.get(buyer).put(item, new CostVarianceData(item, invoice.getTotalPurchasePrice(), invoice.getQuantity(), invoice.getUnitPurchasePrice(), purchaseDate) );
			}
		}
	}
	...
	private List<CostVarianceRow> computeAllCostVarianceByItemRows(Map<Item, CostVarianceData> pastCVDByItemMap, Map<Item, CostVarianceData> currentCVDByItemMap){
		...
		for(Item i : this.itemMap.values() ){
			if(i.isPreviouslyPurchased() ){
				...
				result.add(new CostVarianceRow(i.getItemId(), i.getItemDescription(), currentCVD.getTotalPrice(), null, null, averageCostVariance, averagePercentageCostVariance , lastCostVariance , lastPercentageCostVariance) );
				} else{
					//result.add(new CostVarianceRow(i.getItemId(), i.getItemDescription(), 0.0, null, null, null, null, null, null) );
				}
			}else if(i.isCurrentlyPurchased()){
				//result.add(new CostVarianceRow(i.getItemId(), i.getItemDescription(), currentCVDByItemMap.get(i).getTotalPrice(), null, null, null, null, null, null) );
			}
		}
	...
	public void simulate() throws NotFoundPriceException {
		...
		/*simulator.setProbabilities(this.probabilities);
		simulator.setTrendsByItem(this.trendsByItem);*/
		...
		simulator.init(actual, bom, startDate, probabilitiesToInput, trendsByItem);
		simulator.run();
	}
	...
	public boolean handleNewItemFile() {
		try {
			FileReader fr=new FileReader("newItemFile.csv");
			BufferedReader br=new BufferedReader(fr);
			String line;
			while((line=br.readLine())!=null) {
				String firstLine = "﻿itemId;date (yyyy-mm-dd);quantity;unit price";
				if(!line.equals(firstLine)) {
					try {
						String fields[] = line.split(";");
						String itemId = fields[0];
			...
			while((line=br.readLine())!=null) {
				...
				if(!line.equals(firstLine)) {
					try {
			    		...
			    		if(!this.trendsByItem.containsKey(itemId)) {
			    			pricetrendList = new ArrayList<PriceTrend>();
			    			this.trendsByItem.put(itemId, pricetrendList);
			    		}
			    		else
			    			pricetrendList = this.trendsByItem.get(itemId);
	...
	public void addInActual(LocalDate date, double price, String itemId, int quantity) {
		SimulatorItem si = new SimulatorItem(State.WAREHOUSE_ARRIVAL, date, -1, price, itemId, quantity);
		actual.add(si);
	}
